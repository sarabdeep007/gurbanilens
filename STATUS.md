...
- ✅ **Phase 2A v2 Phase A.4b — cloud ASR providers + Compare debug mode** (2026-06-21) — Sarvam + Gemini + Compare landed in parallel with A.4a's WhisperKit/protocol work; both branches merged cleanly on `ASRProvider` (single locked spec, no contract drift).
  - **`CloudMicCapture`** — sibling to `MicSource`; emits 16 kHz mono s16le PCM `Data` chunks live via `AsyncStream<Data>` (vs MicSource's Float32 bulk-at-stop). Wraps `AVAudioEngine.installTap`, downmixes to mono on the tap thread, AVAudioConverter resamples to 16 kHz, then Float32 → Int16LE in a tight loop. Per-tap peak amplitude exposed for VU display. Co-existence rule: MUST NOT run while WhisperKit's AudioProcessor is alive — StreamingASR's one-provider-per-session contract enforces. Includes a small `WavBuilder.wavFromS16LE` helper that wraps raw PCM in a 44-byte WAV header for Gemini's `audio/wav` inline part.
  - **`SarvamProvider`** — actor conforming to `ASRProvider`. Streaming path opens `wss://api.sarvam.ai/speech-to-text/streaming` with `api-subscription-key` header auth, sends `{type:config, model:saaras:v3, language:pa, encoding:linear16, sample_rate:16000, interim_results:true}`, then forwards CloudMicCapture chunks as binary frames. Receives JSON text frames with `{transcript, is_final, ...}` shape (handles three documented envelope variants — flat, nested `data`, Google-style `results.alternatives`). Auto-detects whether Sarvam emits Gurmukhi or Devanagari per partial and populates `Partial.gurmukhi` directly vs via `Gurmukhi.fromDevanagari`. Latin always derived via `Latin.from` for the matcher. Also exposes a REST batch `transcribeOneShot(wav:apiKey:)` against `/speech-to-text` (multipart/form-data, `model=saaras:v3`, `language_code=pa-IN`) used by CompareScreen.
  - **`GeminiProvider`** — actor conforming to `ASRProvider`. REST chunked: accumulates ~2 sec of s16le PCM per chunk, wraps in WAV, base64-encodes, POSTs `v1beta/models/gemini-2.5-flash:generateContent?key=…` with fixed Gurmukhi-only transcription prompt (no translation, no commentary, no metadata) at temperature 0. Each chunk response is concat-joined with cheap 20-char overlap dedup into a running transcript snapshot; emits `Partial` per chunk so the UI grows the transcript every ~2-4 sec. `transcribeOneShot(wav:apiKey:)` shares the same request shape for CompareScreen reuse.
  - **`StreamingASR` facade wiring** — A.4a's stubbed `.sarvam` / `.gemini` cases that fell back to WhisperKit are replaced with real `SarvamProvider()` / `GeminiProvider()` instantiation. Missing API keys throw `SarvamError.missingApiKey` / `GeminiError.missingApiKey` at start time and propagate via `session.setError`; the user can flip Settings → Cloud OFF and fall back to Whisper.
  - **`.env` → Info.plist build-time injection** — `scripts/inject_env_to_plist.sh` is wired as a `postBuildScript` in `project.yml`. Reads `SARVAM_API_KEY` and `GEMINI_API_KEY` from `.env` at repo root and `PlistBuddy`-writes them as string entries into the built app's `Info.plist`. Runs after Copy Bundle Resources (Info.plist already exists in the .app) and before code signing (so the signature covers our additions). Missing `.env` is a warning, not an error. `.env` was already in `.gitignore`; `.env.example` documents both keys.
  - **Settings → Voice recognition → Cloud sub-section** — Toggle (`settings.cloudEnabled`, default OFF) snaps `settings.asrProvider` back to `.whisperKit` on OFF and defaults to Sarvam on first ON. When ON, a Sarvam / Gemini radio pair lets the user pick the active cloud provider. Free-trial counter ("`N` of 50 cloud searches this month") + privacy note ("audio is sent to the provider"). Counter rolls forward to a fresh 50 on month boundary via `CloudTrialPolicy.resetIfNewMonth` (UTC YYYY-MM key). 5 rapid taps on the Settings → Version footer unlock `settings.debugCompareEnabled`.
  - **`CloudTrialPolicy`** — small helper file owning the `@AppStorage` keys + monthly-reset rule + `tryConsume` transition + `forceDisable`. `AppContainer.commitLiveStream` calls `tryConsume()` BEFORE running the commit when the active provider is cloud; if the trial was exhausted on entry it snaps cloud OFF + surfaces "Free cloud trial used up for this month — switched back to Local Whisper" via the existing error alert path, and bails before the network spend.
  - **`CompareScreen`** — debug A/B test modal sheet, presented from LiveResultsScreen's toolbar Compare button (only visible when `debugCompareEnabled`). Tap Record → MicSource captures up to 10 sec of 16 kHz mono Float32 PCM → on Stop, the audio is fanned out to all three providers in parallel via `async let`: WhisperKit via `WhisperOneShot.transcribe`, Sarvam via `SarvamProvider.transcribeOneShot` (REST batch), Gemini via `GeminiProvider.transcribeOneShot`. Each row shows provider name + elapsed-ms + Gurmukhi transcript + Latin transliteration as soon as it lands. Save Comparison writes a `compare-{ts}.json` payload to `Documents/` for extraction via Xcode → Devices → Download Container. The deciding tool for Deep to commit to a cloud provider.
  - **`CloudParsing` in GurbaniLensCore + 23 new unit tests** — the pure parsing helpers (`detectScript`, `sanitize`, `extractSarvamTranscript`, `extractGeminiText`, `joinAccumulator`) are factored into `GurbaniLensCore.CloudParsing` so they run under `swift test` (the iOS app target has no SPM test bundle). The providers thin-wrap. Coverage: detectScript pure / mixed-majority / tie / non-Indic / empty; sanitize transcript-prefix strip / triple-backtick fence strip; extractSarvamTranscript flat / text fallback / nested data / Google-style alternatives / sanitize composition / missing-fields-nil / empty-string-nil; extractGeminiText candidate / multi-part / sanitize / error response / empty; joinAccumulator empty prev/next, simple concat, 20-char overlap dedup, no-overlap. Total GurbaniLensCore test count: was 25 with corpus (11 + 6 + 8); now **48 with corpus** (+23), **29 in CI without corpus** (was 14, +23 since CloudParsing tests don't need the corpus).
